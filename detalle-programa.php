...
$restitulo = $db->getRow("SELECT nombre, descripcionInfantil FROM categorias WHERE idCategoria = ? AND baja=0", array($idCategoria));

if (sizeof($restitulo) == 0) {
	echo '<script type="text/javascript">location.href="https://tutorahkids.tv";</script>';
	exit();
}

$titulo = $restitulo['nombre'] . " - TuTorahKids.tv";
$descriptionHeader = $restitulo['nombre'] . ". " . $restitulo['descripcionInfantil'];
...
$titvideo = $restitulo['nombre'];
$video = $videouri[0];
$categoria_url = strtolower(preg_replace('/[^a-zA-Z0-9]+/', '-', trim($restitulo['nombre'])));
...
<body>
	...
	<section class="container-fluid detalle-video mt-video" style="background-image: url(<?php echo URL_BACKGROUND . $idCategoria . '_infantil.png'; ?>);">
		...
		<div class="row justify-content-center ads-int">
			<div class="col-12 col-lg-10 text-center px-4 p-md-3 my-3 my-md-0">

                <!-- Revive Adserver Asynchronous JS Tag - Generated with Revive Adserver v4.2.1 -->
                <ins data-revive-zoneid="61" data-revive-target="_blank" data-revive-block="1" data-revive-blockcampaign="1" data-revive-id="b66a012a1acd0d45703781284cf2d38d"></ins>
	...
	<section id="detalle-programacion" class="container mt-0 mt-lg-3">
	</section>
	...
	<script type="text/javascript">
		...
		let programaUrl = '<?php echo $categoria_url ?>';
						...
						$.each(videos, function(i, item) {
							if (item !== undefined) {
								...
								for (let i = 0; i < videoscat[item].length; i++) {
									if (itemsvisibles < 12) {
										elemento += '<div class="col-6 col-md-3 col-lg-3 p-1 col-img __detalle__programa" id="video_' + item + '_' + i + '"><a class="imageBox" href="ver-programa?' + programaUrl + '&vid=' + videoscat[item][i].idMultix + '"><img class="img-fluid preViewTube" src="' + URL_THUMBNAILS + videoscat[item][i].idMultix + '_' + videoscat[item][i].thumbnail + '.jpg" alt=""><div class="textBox"><div class="icon-play"></div><p class="d-block text-truncate--">' + videoscat[item][i].titulo + '<span>' + videoscat[item][i].duracion + '</span></p></div></a></div>';
